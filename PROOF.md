...
| The Act mission really drives the deployment lab's MCP tools | `bash scripts/demo_mission_s3.sh` (needs the [lab](https://github.com/dbett4/hermes-enterprise-deployment-lab) cloned side by side) | `MISSION_S3_DEMO_PASS`: the mutating tool is absent from the read/plan allowlist, an unapproved request writes nothing, a separate operator command grants the capability, an injected post-commit 500 is survived, resume replays, and the store holds exactly one record | The approval identity is a fixture string, the target is the lab's fixture action store, and no model chose these tool calls |
| That wiring is exercised, not just documented | `pytest -q tests/test_deployment_lab_act.py` | The end-to-end test runs the real arc and the oracle rejects a double write, an unapproved side effect, and a leaked capability | The end-to-end cases skip when the lab is not resolvable; `proof.sh` then prints `FIELD_KIT_PROOF_ACT_SKIPPED` |
